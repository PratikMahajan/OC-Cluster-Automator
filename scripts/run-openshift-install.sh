...
# USAGE
#
#    run-openshift-install.sh -s STATE_DIR -a ACTION -n NAME -p PLATFORM
#
# OPTIONS
#
...
#    -p PLATFORM     Platform to initialize cluster on (AWS/Azure)
#
#?
...
case "$action" in
	...
	if ! "$prog_dir/openshift-install-create-config.yaml.sh" -n "$name" -p "$platform" > "$config_f"; then
	    die "Failed to create openshift-install configuration file"
	fi

	echo "Created openshift-install configuration"
	echo "Generating openshift-install manifests"

	if ! openshift-install create manifests --dir "$cluster_d"; then
	    die "Failed to create cluster $name"
	fi

	hybrid_ol="$cluster_d/manifests/cluster-network-03-config.yml"
	if ! "$prog_dir/openshift-install-hybrid-overlay-config.yaml.sh" > "$hybrid_ol"; then
	    die "Failed to create hybrid overlay manifests file"
	fi

	echo "Created openshift-install manifests"
